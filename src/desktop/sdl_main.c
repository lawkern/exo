...
static void sdl_toggle_fullscreen(SDL_Window *window)
{
   ...
   SDL_SetWindowFullscreen(window, !already_fullscreen);
}
...
static bool sdl_frame_begin(sdl_context *sdl, desktop_input *input, texture backbuffer)
{
   ...
   for(u32 key_index = 0; key_index < INPUT_KEY_COUNT; ++key_index)
   {
      input->keys[key_index].changed_state = false;
   }
   ...
   while(SDL_PollEvent(&event))
   {
      switch(event.type)
      {
         ...
         case SDL_EVENT_MOUSE_BUTTON_UP:
         {
            ...
            {
               case SDL_BUTTON_LEFT:   {mouse_button = input->keys + INPUT_KEY_MBLEFT;} break;
               case SDL_BUTTON_MIDDLE: {mouse_button = input->keys + INPUT_KEY_MBMIDDLE;} break;
               case SDL_BUTTON_RIGHT:  {mouse_button = input->keys + INPUT_KEY_MBRIGHT;} break;
               case SDL_BUTTON_X1:     {mouse_button = input->keys + INPUT_KEY_MBX1;} break;
               case SDL_BUTTON_X2:     {mouse_button = input->keys + INPUT_KEY_MBX2;} break;
            }
            assert(mouse_button);
         ...
         case SDL_EVENT_KEY_UP:
         {
            ...
            if(pressed && !repeated)
            {
               if(code == SDLK_TAB)
               {
                  input->keys[INPUT_KEY_TAB].is_pressed = pressed;
                  input->keys[INPUT_KEY_TAB].changed_state = true;
               }
               ...
               {
                  sdl_toggle_fullscreen(sdl->window);
               }
            }
         } break;
      }
   ...
   input->mousex = (int)((float)mousex * ((float)backbuffer.width / (float)window_width));
   input->mousey = (int)((float)mousey * ((float)backbuffer.height / (float)window_height));
